...
s32_t SPIFFS_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  ...
  if (fd->sizet == SPIFFS_UNDEFINED_LEN && len > 0) {
  ...
  if (fd->fdoffset + len >= fd->sizet) {
    // reading beyond file size
    s32_t avail = fd->sizet - fd->fdoffset;
    if (avail <= 0) {
...
static s32_t spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf, u32_t offset, s32_t len) {
  ...
  s32_t res = SPIFFS_OK;
  s32_t remaining = len;
  if (fd->sizet != SPIFFS_UNDEFINED_LEN && offset < fd->sizet) {
    s32_t m_len = MIN((s32_t)(fd->sizet - offset), len);
...
s32_t SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  ...
  if ((fd->flags & SPIFFS_O_APPEND)) {
    fd->fdoffset = fd->sizet == SPIFFS_UNDEFINED_LEN ? 0 : fd->sizet;
  }
...
#endif
  if (fd->flags & SPIFFS_O_APPEND) {
    if (fd->sizet == SPIFFS_UNDEFINED_LEN) {
      offset = 0;
    } else {
      offset = fd->sizet;
    }
#if SPIFFS_CACHE_WR
    if (fd->cache_page) {
      offset = MAX(offset, fd->cache_page->offset + fd->cache_page->sizet);
    }
#endif
  }
    ...
    if (len < (s32_t)SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
      ...
      if (fd->cache_page) {
        ...
        if (offset < fd->cache_page->offset || // writing before cache
            offset > fd->cache_page->offset + fd->cache_page->sizet || // writing after cache
            offset + len > fd->cache_page->offset + SPIFFS_CFG_LOG_PAGE_SZ(fs)) // writing beyond cache page
        {
          ...
          SPIFFS_CACHE_DBG("CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, boundary viol, offs:%i size:%i\n",
              fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->offset, fd->cache_page->sizet);
          res = spiffs_hydro_write(fs, fd,
              spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix),
              fd->cache_page->offset, fd->cache_page->sizet);
      ...
      if (fd->cache_page) {
        ...
        fd->cache_page->sizet = MAX(fd->cache_page->sizet, offset_in_cpage + len);
        fd->fdoffset += len;
        SPIFFS_UNLOCK(fs);
        return len;
      } else {
...
s32_t SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence) {
  ...
  case SPIFFS_SEEK_END:
    offs = (fd->sizet == SPIFFS_UNDEFINED_LEN ? 0 : fd->sizet) + offs;
    break;
  }

  if ((offs > (s32_t)fd->sizet) && (SPIFFS_UNDEFINED_LEN != fd->sizet)) {
    res = SPIFFS_ERR_END_OF_OBJECT;
  }
  SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
...
static s32_t spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s) {
  ...
  s->sizet = objix_hdr.size == SPIFFS_UNDEFINED_LEN ? 0 : objix_hdr.size;
  s->pix = pix;
...
s32_t SPIFFS_eof(spiffs *fs, spiffs_file fh) {
  ...
  res = (fd->fdoffset >= (fd->sizet == SPIFFS_UNDEFINED_LEN ? 0 : fd->sizet));
