  ...
  while (res == SPIFFS_OK && written < len) {
    ...
    if (cur_objix_spix != prev_objix_spix) {
      ...
      if (cur_objix_spix == 0) {
        ...
        spiffs_span_ix len_objix_spix = SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs, (fd->sizet-1)/SPIFFS_DATA_PAGE_SIZE(fs));
...
s32_t spiffs_object_truncate(
  ...
  if ((fd->sizet == SPIFFS_UNDEFINED_LEN || fd->sizet == 0) && !remove_full) {
    // no op
    return res;
  }
  ...
  spiffs_span_ix data_spix = (fd->sizet > 0 ? fd->sizet-1 : 0) / SPIFFS_DATA_PAGE_SIZE(fs);
  u32_t cur_size = fd->sizet == (u32_t)SPIFFS_UNDEFINED_LEN ? 0 : fd->sizet ;
  spiffs_span_ix cur_objix_spix = 0;
  spiffs_span_ix prev_objix_spix = (spiffs_span_ix)-1;
  spiffs_page_object_ix_header *objix_hdr = (spiffs_page_object_ix_header *)fs->work;
  spiffs_page_object_ix *objix = (spiffs_page_object_ix *)fs->work;
...
s32_t spiffs_object_read(
  ...
  while (cur_offset < offset + len) {
    ...
    len_to_read = MIN(len_to_read, fd->sizet);
